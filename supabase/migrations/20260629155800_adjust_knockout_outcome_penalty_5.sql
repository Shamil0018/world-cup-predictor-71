-- Drop and recreate public.leaderboard view with group stage best N-3 logic, regular group scores, and double-weighted knockout scoring (50 - 2.5 * error - 5 if wrong winner)
...
user_knockout_predictions as (
  select
    ...
    sum(
      ...
      - (case when pr.predicted_winner_id = m.winner_id then 0 else 5 end)
    )::numeric as points,

-- Drop and recreate public.leaderboard view with group stage best N-3 logic, halved group scores, and direct knockout scoring (20 - error)
...
user_knockout_predictions as (
  select
    pr.user_id,
    sum(20 - public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score, m.stage, pr.predicted_winner_id, m.winner_id))::numeric as points,
...
select
  ...
  p.username,
  p.avatar_url,
  (coalesce(ug.total_points, 0) / 2.0 + coalesce(uk.points, 0))::numeric as total_points,

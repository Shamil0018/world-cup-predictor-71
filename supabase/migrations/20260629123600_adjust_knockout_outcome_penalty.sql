-- Drop and recreate public.leaderboard view with group stage best N-3 logic, regular group scores, and double-weighted knockout scoring (50 - 2.5 * error - 10 if wrong winner)
...
with user_group_predictions as (
  select
    pr.user_id,
    (20 - public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score, m.stage, pr.predicted_winner_id, m.winner_id))::numeric as points,
...
user_knockout_predictions as (
  select
    pr.user_id,
    sum(
      50 - 2.5 * public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score, m.stage, pr.predicted_winner_id, m.winner_id)
      - (case when pr.predicted_winner_id = m.winner_id then 0 else 10 end)
    )::numeric as points,
...
select
  ...
  p.username,
  p.avatar_url,
  (coalesce(ug.total_points, 0) + coalesce(uk.points, 0))::numeric as total_points,

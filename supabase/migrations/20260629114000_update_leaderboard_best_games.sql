-- Drop and recreate public.leaderboard view with group stage best N-3 logic and direct knockout scoring
...
with user_group_predictions as (
  select
    pr.user_id,
    (20 - public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score)) as points,
    public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score) as error,
    row_number() over (
      partition by pr.user_id 
      order by (20 - public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score)) desc, pr.id asc
    ) as rnk,
...
user_knockout_predictions as (
  select
    pr.user_id,
    sum(20 - public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score))::int as points,
    sum(public.prediction_error(pr.predicted_home, pr.predicted_away, m.home_score, m.away_score))::int as error,
